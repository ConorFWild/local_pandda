...
def get_affinity_background_corrected_density(
    ...
    dataset_min_x = max([0, index[0]-math.floor(fragment_shape[0]/2)])
    dataset_min_y = max([0, index[1]-math.floor(fragment_shape[1]/2)])
    dataset_min_z = max([0, index[2]-math.floor(fragment_shape[2]/2)])
    dataset_max_x = min([dataset_shape[0], index[0]+math.floor(fragment_shape[0]/2)+1])
    dataset_max_y = min([dataset_shape[1], index[1]+math.floor(fragment_shape[1]/2)+1])
    dataset_max_z = min([dataset_shape[2], index[2]+math.floor(fragment_shape[2]/2)+1])
    ...
    fragment_min_x = max([0,  math.floor(fragment_map.shape[0]/2)-index[0]])
    fragment_min_y = max([0,  math.floor(fragment_map.shape[1]/2)-index[1]])
    fragment_min_z = max([0,  math.floor(fragment_map.shape[2]/2)-index[2]])
    fragment_max_x = min([fragment_shape[0], math.floor(fragment_shape[0]/2)+1+(dataset_shape[0]-index[0])])
    fragment_max_y = min([fragment_shape[1], math.floor(fragment_shape[1]/2)+1+(dataset_shape[1]-index[1])])
    fragment_max_z = min([fragment_shape[2], math.floor(fragment_shape[2]/2)+1+(dataset_shape[2]-index[2])])
